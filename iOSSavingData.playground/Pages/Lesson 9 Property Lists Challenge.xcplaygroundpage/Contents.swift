...
do {
    ...
    let propertyData = try propertyListEncoder.encode(scenes)
    try propertyData.write(to: plistURL)
    
    let propertyListDecoder = PropertyListDecoder()
    let savedPropertyData = try Data(contentsOf: plistURL)
    let propertyScenes = try propertyListDecoder.decode([Scene].self, from: savedPropertyData)
    
    scenes == propertyScenes
    
    jsonScenes == propertyScenes
